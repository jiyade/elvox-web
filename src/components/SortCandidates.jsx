...
const SortCandidates = ({ sort, setSort }) => {
    return (
        <Listbox
            ...
            onChange={setSort}
        >
            <ListboxButton className='flex items-center justify-between gap-2 cursor-pointer w-full bg-field-light dark:bg-field-dark rounded-md p-2 flex-1 focus:outline-none text-primary-light dark:text-primary-dark'>
                <span className='text-sm'>Sort</span>
                <HiChevronDown className='size-5' />
            </ListboxButton>
            <ListboxOptions className='flex flex-col border border-gray-500 rounded-md focus:outline-none text-sm absolute w-full top-12 z-10 bg-bg-light dark:bg-bg-dark'>
                <ListboxOption
                    value='name'
