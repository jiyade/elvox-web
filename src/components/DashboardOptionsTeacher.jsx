...
const DashboardOptionsTeacher = ({ hasTie }) => {
    const {
        user: { tutor_of }
    } = useAuthStore()
    ...
    return (
        ...
        >
            ...
            <Link
                to='/appeals'
                className={`${tutor_of === null || hasTie ? "max-lg:col-span-2" : ""} ${hasTie ? "lg:hidden" : ""}`}
            >
                <Button className='flex flex-col justify-center items-center py-5 lg:py-7 gap-1 bg-accent hover:bg-button-hover w-full h-full'>
            ...
            {hasTie && (
                <div className='grid grid-cols-2 col-span-3 gap-x-3 max-lg:hidden'>
                    <Link to='/results'>
                        <Button className='flex flex-col justify-center items-center py-5 lg:py-7 gap-1 bg-accent hover:bg-button-hover w-full h-full'>
                            <FiAward className='text-primary-dark text-base lg:text-lg' />
                            <span className=''>View Results</span>
